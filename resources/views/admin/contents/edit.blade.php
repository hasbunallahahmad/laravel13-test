            ...
            <div class="space-y-6">
                ...
                <div class="rounded-xl border border-app-border bg-app-surface p-6">
                    <div class="mb-6">
                        ...
                        <flux:text class="mt-1">
                            {{ __('Update the main information for this content.') }}
                        </flux:text>
                    </div>
                    ...
                    <div class="grid gap-6">
                        ...
                        <flux:field>
                            ...
                            <flux:select name="type" id="type" required>
                                @foreach (\App\Enums\ContentType::cases() as $type)
                                    <option value="{{ $type->value }}" @selected(old('type', $content->type->value) === $type->value)>
                                        {{ ucfirst($type->value) }}
                                    </option>
                                @endforeach
                            </flux:select>
                        ...
                        <flux:field>
                            ...
                            <flux:input name="title" id="title" value="{{ old('title', $content->title) }}"
                                required minlength="3" maxlength="255" data-slug-source />
                        ...
                        <flux:field>
                            ...
                            <flux:input name="slug" id="slug" value="{{ old('slug', $content->slug) }}"
                                required minlength="3" maxlength="255" pattern="[a-z0-9]+(?:-[a-z0-9]+)*"
                                data-slug-target data-original-slug="{{ $content->slug }}" />
                        ...
                        <flux:field>
                            ...
                            <flux:textarea name="excerpt" id="excerpt" rows="4" maxlength="1000">
                                {{ old('excerpt', $content->excerpt) }}</flux:textarea>
                ...
                <div class="rounded-xl border border-app-border bg-app-surface p-6">
                    <div class="mb-6">
                        ...
                        <flux:text class="mt-1">
                            {{ __('Update the content status, author, and publication date.') }}
                        </flux:text>
                    </div>
                        ...
                        <flux:field>
                            ...
                            <flux:select name="status" id="status" required>
                                @foreach (\App\Enums\ContentStatus::cases() as $status)
                                    <option value="{{ $status->value }}" @selected(old('status', $content->status->value) === $status->value)>
                        ...
                        <flux:field>
                            ...
                            <flux:select name="author_uuid" id="author_uuid">
                                ...
                                @foreach ($authors as $author)
                                    <option value="{{ $author->uuid }}" @selected(old('author_uuid', $content->author?->uuid) === $author->uuid)>
                                        {{ $author->name }}
                                    </option>
                                @endforeach
                            </flux:select>
